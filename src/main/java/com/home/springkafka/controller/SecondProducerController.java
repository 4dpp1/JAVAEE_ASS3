...
public class SecondProducerController {
    ...
    @ResponseBody
    @PostMapping(value = "/second-produce", produces = "application/json")
    public String produce(@RequestBody Object obj) {
        try {
            String obj2String = JSONUtil.toJsonStr(obj);
            kafkaTemplate.send(SpringBootKafkaConfig.TOPIC_TEST_SECOND, obj2String);
            return "Second producer success";
        } catch (Exception e) {
            log.error("Error producing message: {}", e.getMessage());
            return "Second producer error";
        }
    }
}
